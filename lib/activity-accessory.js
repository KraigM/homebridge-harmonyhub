...
ActivityAccessory.prototype.updateConnection = function() {
	...
	if (oldConn != newConn) {
		if (oldConn) this._unhookFromConnection(oldConn);
		if (newConn) this._hookIntoConnection(newConn);
	}
	return rtn;
};
ActivityAccessory.prototype._hookIntoConnection = function(conn) {
	conn.addListener(HubConnection.Events.ConnectionChanged, this._onConnectionChanged);
	conn.addListener(HubConnection.Events.StateDigest, this._onStateChanged);
};
ActivityAccessory.prototype._unhookFromConnection = function(conn) {
	conn.removeListener(HubConnection.Events.ConnectionChanged, this._onConnectionChanged);
	conn.removeListener(HubConnection.Events.StateDigest, this._onStateChanged);
};
...
var onStateChanged = function(args) {
	...
	//var activityStatus = stateDigest && stateDigest.activityStatus;
